...
func (backFiller grabBackFiller) BackFill(startingBlock int) error {
	...
	for i, grab := range grabs {
		err := shared.FetchAndPersistDartDinkDiffs(grab.Dart, grab.Dink, grab.UrnID, grab.HeaderID, backFiller.eventsRepository, backFiller.storageRepository, backFiller.blockChain)
		if err != nil {
