...
CREATE OR REPLACE FUNCTION notify_pip_log_value() RETURNS trigger AS $$
BEGIN
  PERFORM pg_notify(
...
CREATE TRIGGER notify_pip_log_value
  AFTER INSERT ON maker.pip_log_value
  FOR EACH ROW
  EXECUTE PROCEDURE notify_pip_log_value();


-- +goose Down
DROP TRIGGER notify_pip_log_value ON maker.pip_log_value;
