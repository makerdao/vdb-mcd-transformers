...
var _ = Describe("jug storage keys loader", func() {
	...
	BeforeEach(func() {
		storageRepository = &test_helpers.MockMakerStorageRepository{}
		storageKeysLoader = jug.NewKeysLoader(storageRepository, test_data.JugAddress())
	})
	...
	Describe("wards", func() {
		It("returns value metadata for wards", func() {
			...
			Expect(storageRepository.GetWardsKeysCalledWith).To(Equal(test_data.JugAddress()))
