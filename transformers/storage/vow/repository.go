...
func (repository VowStorageRepository) Create(diffID, headerID int64, metadata vdbStorage.StorageValueMetadata, value interface{}) error {
...
func (repository VowStorageRepository) insertSinMapping(diffID, headerID int64, metadata vdbStorage.StorageValueMetadata, sin string) error {
...
func getTimestamp(keys map[vdbStorage.Key]string) (string, error) {
	timestamp, ok := keys[constants.Timestamp]
	if !ok {
		return "", vdbStorage.ErrMetadataMalformed{MissingData: constants.Timestamp}
	}
