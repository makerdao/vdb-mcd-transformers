...
var _ = Describe("Cat storage keys loader", func() {
	...
	BeforeEach(func() {
		storageRepository = &test_helpers.MockMakerStorageRepository{}
		storageKeysLoader = v1_1_0.NewKeysLoader(storageRepository, test_data.Cat110Address())
	})
	...
	It("returns value metadata for static keys", func() {
		...
		Expect(mappings[v1_1_0.BoxKey]).To(Equal(v1_1_0.BoxMetadata))
		Expect(mappings[v1_1_0.LitterKey]).To(Equal(v1_1_0.LitterMetadata))
	})
	...
	Describe("ilk", func() {
		...
		Describe("when getting ilks succeeds", func() {
			...
			It("returns value metadata for ilk dunk", func() {
				ilkDunkKey := vdbStorage.GetIncrementedKey(ilkFlipKey, 2)
				expectedMetadata := types.ValueMetadata{
					Name: cat.IlkDunk,
					Keys: map[types.Key]string{constants.Ilk: test_helpers.FakeIlk},
					Type: types.Uint256,
				...
				Expect(mappings[ilkDunkKey]).To(Equal(expectedMetadata))
			})
		})
	})
	...
	Describe("wards", func() {
		It("returns value metadata for wards", func() {
			...
			Expect(storageRepository.GetWardsKeysCalledWith).To(Equal(test_data.Cat110Address()))
