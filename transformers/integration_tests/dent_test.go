...
var _ = Describe("Dent transformer", func() {
	...
	BeforeEach(func() {
		...
		dentConfig = event.TransformerConfig{
			...
			ContractAddresses: append(test_data.Flip100Addresses(), test_data.FlopV101Address()),
	...
	It("persists a flip dent log event", func() {
		...
		flipContractAddressId, err := shared.GetOrCreateAddress(test_data.FlipEthAV100Address(), db)
		Expect(err).NotTo(HaveOccurred())
