...
var _ = Describe("Executing the flip transformer", func() {
	...
	It("reads in a vat storage diff and persists it", func() {
		...
		err = db.Get(&vatResult,
			`SELECT header_id, vat AS value FROM maker.flip_vat`)
		Expect(err).NotTo(HaveOccurred())
	...
	It("reads in an ilk storage diff and persists it", func() {
		...
		err = db.Get(&ilkResult,
			`SELECT header_id, ilk_id AS value FROM maker.flip_ilk`)
		Expect(err).NotTo(HaveOccurred())
		ilkID, ilkErr := shared.GetOrCreateIlk(ilk, db)
		Expect(ilkErr).NotTo(HaveOccurred())
		test_helpers.AssertVariable(ilkResult, headerID, strconv.FormatInt(ilkID, 10))
	})
	...
	It("reads in a beg storage diff and persists it", func() {
		...
		err = db.Get(&begResult,
			`SELECT header_id, beg AS value FROM maker.flip_beg`)
		Expect(err).NotTo(HaveOccurred())
	...
	It("reads in a ttl storage diff and persists it", func() {
		...
		err = db.Get(&ttlResult,
			`SELECT header_id, ttl AS value FROM maker.flip_ttl`)
		Expect(err).NotTo(HaveOccurred())
	...
	It("reads in a tau storage diff and persists it", func() {
		...
		var ttlResult test_helpers.VariableRes
		err = db.Get(&ttlResult,
			`SELECT header_id, tau AS value FROM maker.flip_tau`)
		Expect(err).NotTo(HaveOccurred())
	...
	Describe("bids", func() {
		...
		Describe("guy + tic + end packed slot", func() {
			...
			BeforeEach(func() {
				...
				_, writeErr := db.Exec(flip.InsertFlipKicksQuery, headerID, addressId, bidId)
			...
			It("reads and persists a guy diff", func() {
				...
				dbErr := db.Get(&bidGuyResult, `SELECT header_id, bid_id AS key, guy AS value FROM maker.flip_bid_guy`)
			...
			It("reads and persists a tic diff", func() {
				...
				dbErr := db.Get(&bidTicResult, `SELECT header_id, bid_id AS key, tic AS value FROM maker.flip_bid_tic`)
				Expect(dbErr).NotTo(HaveOccurred())
			...
			It("reads and persists an end diff", func() {
				...
				dbErr := db.Get(&bidEndResult, `SELECT header_id, bid_id AS key, "end" AS value FROM maker.flip_bid_end`)
				Expect(dbErr).NotTo(HaveOccurred())
