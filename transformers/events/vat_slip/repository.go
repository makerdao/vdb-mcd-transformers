...
func (repository VatSlipRepository) Create(headerID int64, models []interface{}) error {
	...
	for _, model := range models {
		...
		_, execErr := tx.Exec(
			`INSERT into maker.vat_slip (header_id, ilk_id, usr, rad, tx_idx, log_idx, raw_log)
			VALUES($1, $2, $3, $4::NUMERIC, $5, $6, $7)
			ON CONFLICT (header_id, tx_idx, log_idx) DO UPDATE SET ilk_id = $2, usr = $3, rad = $4, raw_log = $7;`,
			headerID, ilkID, vatSlip.Usr, vatSlip.Rad, vatSlip.TransactionIndex, vatSlip.LogIndex, vatSlip.Raw,
		)
		if execErr != nil {
			rollbackErr := tx.Rollback()
			if rollbackErr != nil {
