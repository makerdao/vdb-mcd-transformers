...
var _ = Describe("Vat grab converter", func() {
	...
	It("converts a log with positive dink to a model", func() {
		models, err := converter.ToModels(constants.VatABI(), []types.Log{test_data.EthVatGrabLogWithPositiveDink})
	...
	It("converts a log with negative dink to a model", func() {
		models, err := converter.ToModels(constants.VatABI(), []types.Log{test_data.EthVatGrabLogWithNegativeDink})
